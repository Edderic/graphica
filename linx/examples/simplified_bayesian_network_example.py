"""
Example demonstrating the simplified BayesianNetwork interface.

This example shows how to create a Bayesian network using the new simplified
interface where all nodes are RandomVariables (including CPTs).
"""

import numpy as np
from linx.bayesian_network import BayesianNetwork
from linx.random.normal import Normal
from linx.random.uniform import Uniform
from linx.conditional_probability_table import ConditionalProbabilityTable as CPT
...
def main():
    ...
    for i in range(5):
        particle = bn.sample()
        print(f"Sample {i+1}: X={particle.get_value('X'):.3f}, "
    ...
    for i, sample in enumerate(samples[:5]):
        print(f"  Sample {i+1}: X={sample.get_value('X'):.3f}, "
